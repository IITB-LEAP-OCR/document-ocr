...
def convert_table_to_otsl(html_content, bbox, table_children=None):
    """
    ...
    Args:
        html_content: The HTML content of the table
        bbox: The bounding box coordinates of the table
        table_children: Child blocks that may contain TableCell elements
    """
    ...
    try:
        ...
        table_x1, table_y1, table_x2, table_y2 = bbox if len(bbox) >= 4 else [0, 0, 0, 0]
        
        # Get cell coordinates from TableCell children if available
        cell_coordinates = {}
        if table_children:
            cell_coordinates = process_table_cells(table_children)
        
        # Start building OTSL table representation
        otsl_table = f'<table x1="{table_x1}" y1="{table_y1}" x2="{table_x2}" y2="{table_y2}">\n'
        
        # Extract rows
        rows = table.find_all('tr')
        is_header_row = True  # Assume first row is header
        
        for row_idx, row in enumerate(rows, 1):  # Start at 1 to match the process_table_cells indexing
            # Calculate row coordinates
            row_y1 = table_y1 + ((row_idx-1) * (table_y2 - table_y1) // max(len(rows), 1))
            row_y2 = table_y1 + (row_idx * (table_y2 - table_y1) // max(len(rows), 1))
            
            otsl_table += f'  <tr id="row_{row_idx}" y1="{row_y1}" y2="{row_y2}">\n'
            
            # Extract cells (th and td)
            cells = row.find_all(['th', 'td'])
            for cell_idx, cell in enumerate(cells, 1):  # Start at 1 to match the process_table_cells indexing
                # Get cell content
                cell_content = cell.get_text()
                cell_type = cell.name
                
                # Get coordinates from our mapping if available
                coords = cell_coordinates.get((row_idx, cell_idx), {})
                if coords:
                    cell_x1 = coords.get('x1')
                    cell_y1 = coords.get('y1')
                    cell_x2 = coords.get('x2')
                    cell_y2 = coords.get('y2')
                    # Use content from TableCell if available
                    if 'html' in coords and coords['html']:
                        cell_content_from_coords = BeautifulSoup(coords['html'], 'html.parser').get_text().strip()
                        if cell_content_from_coords:
                            cell_content = cell_content_from_coords
                else:
                    # Calculate default coordinates if not found in TableCell children
                    cell_x1 = table_x1 + ((cell_idx-1) * (table_x2 - table_x1) // max(len(cells), 1))
                    cell_y1 = row_y1
                    cell_x2 = table_x1 + (cell_idx * (table_x2 - table_x1) // max(len(cells), 1))
                    cell_y2 = row_y2
                
                # Determine OTSL cell type
                if not cell_content:
                    # Empty cell
                    otsl_cell_type = "ecell"
                elif cell_type == "th" or (is_header_row and cell_idx == 1):  # cell_idx starts at 1 now
                    # Header cell (either th or first cell in first row for row headers)
                    otsl_cell_type = "ched" if cell_idx > 1 or row_idx == 1 else "rhed"
                else:
                    # Regular non-empty cell
                    otsl_cell_type = "fcell"
                
                # Create OTSL cell with coordinates
                otsl_table += f'    <{otsl_cell_type} id="cell_{row_idx}_{cell_idx}" '
                otsl_table += f'x1="{cell_x1}" y1="{cell_y1}" x2="{cell_x2}" y2="{cell_y2}">'
                otsl_table += f'{cell_content}</{otsl_cell_type}>\n'
...
def convert_to_tags(block):
    ...
    if block_type == 'Table':
        # Get table children that might contain TableCell elements with coordinates
        table_children = block.get('children', [])
        html_content = convert_table_to_otsl(html_content, bbox, table_children)
        
        # We'll process non-TableCell children later
        children_to_process = [child for child in table_children if child.get('block_type') != 'TableCell']
    else:
        children_to_process = block.get('children', [])
