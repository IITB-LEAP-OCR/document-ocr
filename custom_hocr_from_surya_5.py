...
def process_list_group(block):
    """
    Process a list block to remove redundant ListItem tags and keep only the HTML content.
    Works with ListGroup, OrderedList, UnorderedList, or any block containing ListItem children.
    Instead of nesting each <li> inside a ListItem tag, we'll extract all <li> elements
    and place them directly under the parent list tag.
    
    Args:
        block: The list block to process
        
    Returns:
        str: The processed list content with only HTML list items
    """
    # Get the list block's bbox
    bbox = block.get('bbox', [0, 0, 0, 0])
    block_type = block.get('block_type', 'ListGroup')
    
    # Get compressed tag name from layouts dictionary
    compressed_tag = layouts.get(block_type, block_type)
    
    # Initialize the result with list tag and location
    if len(bbox) >= 4:
        loc_tags = f'<loc_{bbox[0]}><loc_{bbox[1]}><loc_{bbox[2]}><loc_{bbox[3]}>'
    else:
        loc_tags = '<loc_0><loc_0><loc_0><loc_0>'
    
    result = f'<{compressed_tag}>\n  {loc_tags}\n'
    
    # Extract all list items from children
    list_items = []
    if 'children' in block and block['children']:
        for child in block['children']:
            if child.get('block_type') == 'ListItem':
                # Extract the <li> content from the child
                html_content = clean_html_content(child.get('html', ''))
                if html_content:
                    # Make sure we have a <li> tag
                    if '<li>' not in html_content and not html_content.startswith('<li '):
                        html_content = f'<li>{html_content}</li>'
                    list_items.append(html_content)
    
    # Add all list items directly to the list
    if list_items:
        # If this is an OrderedList or UnorderedList and there's no explicit ol/ul tag,
        # wrap items in the appropriate list tag
        list_html = ""
        if block_type == 'OrderedList' and not any('<ol' in item for item in list_items):
            list_html = "<ol>\n"
            for item in list_items:
                list_html += f"    {item}\n"
            list_html += "  </ol>"
            result += f"  {list_html}\n"
        elif block_type == 'UnorderedList' and not any('<ul' in item for item in list_items):
            list_html = "<ul>\n"
            for item in list_items:
                list_html += f"    {item}\n"
            list_html += "  </ul>"
...
def convert_to_tags(block):
    """Convert a block to tag-based format using compressed tag names"""
    ...
    # Special handling for any type of list block
    if block_type in ['ListGroup', 'OrderedList', 'UnorderedList', 'List']:
        return process_list_group(block)
    ...
    # Start the tag with compressed name
    result = f'<{compressed_tag}>  {loc_tags}'
    ...
    if 'children' in block and block['children']:
        ...
        # Skip ListItem children for processed list blocks
        elif block_type in ['ListGroup', 'OrderedList', 'UnorderedList', 'List']:
            child_blocks = [child for child in children if child.get('block_type') != 'ListItem']
        else:
    ...
    # Close the tag with compressed name
    result += f' </{compressed_tag}>'
...
def main():
    ...
    print(f"Simplified list structures by removing redundant ListItem tags")
    print(f"Used compressed tags for cleaner output")
