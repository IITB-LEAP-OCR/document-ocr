...
def process_table_cells(table_children):
    """
    Extract coordinate information from TableCell children to enhance OTSL table
    
    Args:
        table_children: List of child blocks (TableCell elements)
        
    Returns:
        dict: Mapping of cell positions to their coordinates and content
    """
    ...
    # Filter out non-TableCell children
    table_cells = [cell for cell in table_children if cell.get('block_type') == 'TableCell']
...
def convert_to_tags(block):
    ...
    # Start the tag with compressed name instead of full block_type
...
def main():
    ...
    print(f"Used compressed tags from layouts dictionary for cleaner output")
